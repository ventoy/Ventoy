...
ventoy_dirhook_size (const struct grub_dirhook_info *info)
{
    (void) info;
    return 0;
}
...
static int ventoy_collect_img_files(const char *filename, const struct grub_dirhook_info *info, void *data)
{
    ...
    else
    {
        ...
        if (ventoy_dirhook_size(info) == VTOY_FILT_MIN_FILE_SIZE || ventoy_dirhook_size(info) == 0)
        {
        ...
        if (img)
        {
            ...
            img->size = ventoy_dirhook_size(info);
            if (vlnk || 0 == img->size)
            {
                if (node->dir[0] == '/')
                {
...
static grub_err_t ventoy_cmd_check_secureboot_var(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    if (grub_efi_get_variable("SecureBoot", &global, &size, (void **)&var) == GRUB_EFI_SUCCESS &&
        var && *var == 1)
    {
        grub_free(var);
        return 0;
...
static int ventoy_collect_replace_initrd(const char *filename, const struct grub_dirhook_info *info, void *data)
{
    ...
    else
    {
        ...
        {
            if (ventoy_dirhook_size(info) > 32 * VTOY_SIZE_1MB)
            {
...
static grub_err_t grub_cmd_gptpriority(grub_extcmd_context_t ctxt, int argc, char **args)
{
  ...
  grub_snprintf (priority_str, sizeof(priority_str), "%u", 0U);
