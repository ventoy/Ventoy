...
/* Older GRUB headers in this port track don't carry every MIPS relocation
   constant that current toolchains may emit for 64-bit EFI modules.  Keep the
   compatibility shims local to the verifier until the wider MIPS header drift
   is reconciled. */
