...
static int
is_supported_relocation (const struct grub_module_verifier_arch *arch,
			 grub_uint32_t type, int *is_short)
{
  unsigned i;

  for (i = 0; arch->supported_relocations[i] != -1; i++)
    if (type == (grub_uint32_t) arch->supported_relocations[i])
      {
	*is_short = 0;
	return 1;
      }
  ...
  for (i = 0; arch->short_relocations[i] != -1; i++)
    if (type == (grub_uint32_t) arch->short_relocations[i])
      {
	*is_short = 1;
	return 1;
      }
			   ...
			   Elf_Shdr *s, size_t target_seg_size)
{
  ...
  for (rel = (Elf_Rel *) ((char *) ehdr + grub_target_to_host (s->sh_offset)),
	...
	grub_util_error ("%s: reloc offset is out of the segment", modname);
      ...
      if (arch->machine == EM_MIPS && arch->voidp_sizeof == 8)
	{
	  grub_uint32_t mips_types[] = {
	    r_info & 0xff,
	    (r_info >> 8) & 0xff,
	    (r_info >> 16) & 0xff
	  };
	  int requires_local = 0;

	  for (i = 0; i < ARRAY_SIZE (mips_types); i++)
	    {
	      int is_short = 0;

	      if (mips_types[i] == R_MIPS_NONE)
		continue;
	      if (!is_supported_relocation (arch, mips_types[i], &is_short))
		grub_util_error ("%s: unsupported relocation 0x%x", modname,
				 mips_types[i]);
	      if (is_short)
		requires_local = 1;
	    }
	  ...
	  if (is_symbol_local (sym))
	    continue;
	  grub_util_error ("%s: relocation 0x%x is not module-local", modname,
			   mips_types[0]);
	}
	...
	if (!is_supported_relocation (arch, type, &is_short))
